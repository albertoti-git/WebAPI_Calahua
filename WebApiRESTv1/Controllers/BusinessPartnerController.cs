        ...
        public HttpResponseMessage post([FromBody] BusinessPartner bp)
        {
            ...
            try
            {
                if (!oSocios.GetByKey(bp.CardCode))
                {
                    ...
                    oSocios.PayTermsGrpCode = bp.PayTermsGrpCode;
                ...
                if (bp.Addresses != null && bp.Addresses.Count() > 0)
                {
                    for (int iIndex3 = 0; iIndex3 < bp.Addresses.Count(); iIndex3++)
                    {
                        BPAddress oDir = bp.Addresses[iIndex3];
                        // Buscar si ya existe una dirección con el mismo nombre
                        bool addressExists = false;
                        for (int j = 0; j < oSocios.Addresses.Count; j++)
                        {
                            oSocios.Addresses.SetCurrentLine(j);
                            if (oSocios.Addresses.AddressName == oDir.AddressName)
                            {
                                addressExists = true;
                                break;
                            }
                        }
                        // Si no existe, agregamos una nueva línea
                        if (!addressExists)
                        {
                            oSocios.Addresses.Add();
                        ...
                        // Asignamos valores (en ambos casos: update o add)
                        oSocios.Addresses.AddressType = (oDir.AddressType == "bo_ShipTo")
                    ...
                    businessPartner.UserFields = new Dictionary<string, string>[]
{
    new Dictionary<string, string>()
                    ...
                    List<Models.ContactEmployees> contactos = new List<Models.ContactEmployees>();
                    bool existeCorreo = false;

                    // Primero recorremos para verificar si existe el correo
                    for (int iIndex2 = 0; iIndex2 < oSocios.ContactEmployees.Count; iIndex2++)
                    {
                        oSocios.ContactEmployees.SetCurrentLine(iIndex2);

                        if (!string.IsNullOrEmpty(oSocios.ContactEmployees.E_Mail) &&
                            oSocios.ContactEmployees.E_Mail.Equals(Mail, StringComparison.OrdinalIgnoreCase))
                        {
                            existeCorreo = true;
                            break;
                        }
                    ...
                    // Ahora llenamos la lista de contactos según la condición
                    for (int iIndex2 = 0; iIndex2 < oSocios.ContactEmployees.Count; iIndex2++)
                    {
                        oSocios.ContactEmployees.SetCurrentLine(iIndex2);

                        // Si existe el correo y este no coincide, saltamos
                        if (existeCorreo &&
                            !oSocios.ContactEmployees.E_Mail.Equals(Mail, StringComparison.OrdinalIgnoreCase))
                            continue;

                        var contacto = new Models.ContactEmployees
                        {
                            Name = oSocios.ContactEmployees.Name,
                            Position = oSocios.ContactEmployees.Position,
                            Address = oSocios.ContactEmployees.Address,
                            Phone1 = oSocios.ContactEmployees.Phone1,
                            Phone2 = oSocios.ContactEmployees.Phone2,
                            MobilePhone = oSocios.ContactEmployees.MobilePhone,
                            Fax = oSocios.ContactEmployees.Fax,
                            E_Mail = oSocios.ContactEmployees.E_Mail,
                            Pager = oSocios.ContactEmployees.Pager,
                            Remarks1 = oSocios.ContactEmployees.Remarks1,
                            Title = oSocios.ContactEmployees.Title,
                            FirstName = oSocios.ContactEmployees.FirstName,
                            MiddleName = oSocios.ContactEmployees.MiddleName,
                            LastName = oSocios.ContactEmployees.LastName
                        };

                        if (oSocios.ContactEmployees.DateOfBirth != DateTime.MinValue)
                            contacto.DateOfBirth = oSocios.ContactEmployees.DateOfBirth;

                        // Si usas el campo Gender en SAP (BoGenderTypes)
                        // contacto.Gender = (BoGenderTypes)oSocios.ContactEmployees.Gender;

                        contactos.Add(contacto);
                    }

                    // Asignamos el resultado al objeto final
                    businessPartner.ContactEmployees = contactos.ToArray();
                    ...
                    //if (Mail == oSocios.ContactEmployees.E_Mail)
                    //{ 
                    //}
                    //else { 
