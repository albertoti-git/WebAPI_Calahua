...
namespace WebApiRESTv1.Repositories
{
    public class OrderRepository : IOrderRepository
    {
        ...
        public async Task<Response<List<OrderDto>>> GetOrdersAsync(string docEntry, string cardCode)
        {
            ...
            try
            {
                ...
                using (var command = new SqlCommand("Sp_AYB_WebAPI", connection))
                {
                    ...
                    command.Parameters.Add("@Opc", SqlDbType.Int).Value = 9;
                    ...
                    {
                        order.OrderDetails = details.Where(d => d.DocEntry == order.DocEntry).ToList();
                    }
                    response.Mensaje = "Operación Completada, ejecución exitosa";
                    response.IsSuccess = true;
                    response.Dato = orders;
                }
            }
            catch (Exception ex)
            {
                response.Mensaje = "Error al obtener los pedidos";
